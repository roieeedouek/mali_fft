/*
 * Mali GPU FFT Host Code (OpenCL C API version)
 * 
 * This version uses the standard OpenCL C API instead of C++ bindings
 * to avoid dependency on cl.hpp which may not be in all Yocto builds.
 * 
...
struct FFTConfig {
    ...
    size_t work_group_size;      // 64 for Mali-G31
...
std::string loadKernelSource(const char* filename) {
    ...
    if (!file.is_open()) {
        ...
        std::cerr << "Make sure mali_fft_optimized.cl is in the current directory or" << std::endl;
        std::cerr << "in /usr/local/share/opencl/kernels/" << std::endl;
        return "";
    }
...
int main() {
    ...
    std::cout << "Mali GPU FFT for Renesas RZ/G2L (Mali-G31)" << std::endl;
    std::cout << "==========================================" << std::endl;
    ...
    // Find ARM Mali platform
    cl_platform_id mali_platform = NULL;
    for (cl_uint i = 0; i < num_platforms; i++) {
        char vendor[128];
        err = clGetPlatformInfo(platforms[i], CL_PLATFORM_VENDOR, sizeof(vendor), vendor, NULL);
        if (err == CL_SUCCESS && strstr(vendor, "ARM") != NULL) {
            mali_platform = platforms[i];
            char name[128];
            clGetPlatformInfo(mali_platform, CL_PLATFORM_NAME, sizeof(name), name, NULL);
            std::cout << "Found ARM Mali platform: " << name << std::endl;
            break;
        }
    }
    
    if (mali_platform == NULL) {
        std::cerr << "ARM Mali platform not found!" << std::endl;
        return 1;
    }
    
    // Get GPU device
    cl_device_id device;
    err = clGetDeviceIDs(mali_platform, CL_DEVICE_TYPE_GPU, 1, &device, NULL);
    ...
    if (kernel_source.empty()) {
        ...
        kernel_source = loadKernelSource("/usr/local/share/opencl/kernels/mali_fft_optimized.cl");
        if (kernel_source.empty()) {
            return 1;
        }
    }
